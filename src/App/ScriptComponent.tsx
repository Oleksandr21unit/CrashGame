...
const ScriptComponent = () => {
    ...
    const cashOutBtn = document.getElementById('cash-out-btn') as HTMLButtonElement;
    ...
    const multiplierDisplay = document.getElementById('multiplier') as HTMLElement;
    const result = document.getElementById('result') as HTMLElement;
    const timerDisplay = document.getElementById('timer') as HTMLElement;
    const balanceDisplay = document.getElementById('balance-display') as HTMLElement; // Define balanceDisplay
    const resetBtn = document.getElementById('reset-btn') as HTMLButtonElement;
    const resultDisplay = document.getElementById('results-list') as HTMLElement;
    ...
    function displayResult(bet: number, multiplier: number, winnings: number, outcome: string) {
    ...
    function displayLeaderboard() {
        ...
        const sortedPlayers = Object.values(players).sort((a: any, b: any) => b.biggestWin - a.biggestWin);
    ...
    const betBtnOnclick = () => {
        if (!isMoving) { // Check if the game is not in progress
            ...
            if (!isNaN(betInput) && validateBetInput(betInput)) {
                if (betInput > balance) {
    ...
    <>
        ...
        <div className="button-container">
            <button onClick={betBtnOnclick} id="bet-btn">Place Bet</button>
            <button onClick={cashOutBtnOnclick} id="cash-out-btn">Cash Out!</button>
            <button onClick={resetBtnOnclick} id="reset-btn" style={{display: 'none'}}>Reset</button>
        </div>
        <input
            type="number"
            id="bet-input"
            ...
            min={0}
            value={betInput || undefined}
            onChange={(event: any) => {console.log(event, typeof Event); setBetInput(event.target.value)}}
        />
        <p id="multiplier">Multiplier: 1.00x</p>
        <p id="timer">Time: 0s</p>
        <p id="result"></p>
        <p id="balance-display">Balance: 5.00 credits</p>
        </div>    
        {/* <!-- Results section --> */}
